        ...
        <main style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column' }} className="scrollbar-hide">
          ...
          <div style={{ padding: '0.875rem 1.5rem', borderBottom: '1px solid rgba(255,255,255,0.05)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: '#060b12', flexShrink: 0 }}>
            <div>
              ...
              <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>Welcome back, {user.name || 'Kelvin'}. AI predictions for the next 5 days.</p>
            </div>
          ...
          <div style={{ padding: '1.25rem 1.5rem', flex: 1 }}>
                ...
                <div style={{ background: '#0c1420', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '12px', overflow: 'hidden' }}>
                  ...
                  <div style={{ padding: '1.5rem 1.25rem' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '4px', marginBottom: '1rem' }}>
                      {[
                        { label: 'HOME WIN', pct: 58, odds: featured.homeOdds || 1.72, color: '#3b82f6' },
                        { label: 'DRAW', pct: 22, odds: featured.drawOdds || 3.50, color: '#8b5cf6' },
                        { label: 'AWAY WIN', pct: 20, odds: featured.awayOdds || 4.80, color: '#ef4444', isAway: true },
                      ].map((o, i) => (
                    ...
                    <div style={{ background: 'rgba(52,211,153,0.04)', border: '1px solid rgba(52,211,153,0.12)', borderRadius: '8px', padding: '0.75rem 1rem', display: 'flex', gap: '0.625rem', alignItems: 'flex-start' }}>
                      ...
                      <div>
                        ...
                        <span style={{ fontSize: '0.72rem', color: 'var(--text-secondary)' }}>{featured.aiAnalysis || `${featured.homeTeam} dominates with strong home form. AI projects a Home Win at ${featured.homeOdds || 1.72} as the highest value pick.`}</span>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div style={{ background: '#0c1420', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '12px', overflow: 'hidden' }}>
                  ...
                  <div style={{ padding: '0.75rem' }}>
                    {predictions.filter(p => p.confidence >= 75).slice(0, 4).map((p: any, i: number) => {
                      const edge = ((p.confidence / 100 - 1 / (p.homeOdds || p.odds || 2)) * 100).toFixed(1);
                      ...
                      return (
                        <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0.75rem', borderRadius: '8px', marginBottom: '0.5rem', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.04)', cursor: 'pointer', transition: 'all 0.15s' }}
                          onClick={() => addBet(p, getOutcomeKey(p.prediction), p.prediction === 'home_win' ? (p.homeOdds || 1.90) : p.prediction === 'away_win' ? (p.awayOdds || 3.50) : (p.drawOdds || 3.20))}>
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flexShrink: 0 }}>
                            ...
                            <div style={{ background: '#fbbf24', color: '#020408', borderRadius: '6px', padding: '0.3rem 0.625rem', fontFamily: 'var(--font-mono)', fontWeight: 700, fontSize: '0.82rem', minWidth: '44px', textAlign: 'center' }}>
                              {(p.prediction === 'home_win' ? (p.homeOdds || 1.90) : p.prediction === 'away_win' ? (p.awayOdds || 3.50) : (p.drawOdds || 3.20)).toFixed(2)}
                            </div>
                          </div>
                        </div>
                      );
            ...
            <div style={{ background: '#0c1420', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '12px', overflow: 'hidden' }}>
              ...
              {isLoading ? (
                ...
                <div style={{ padding: '3rem', textAlign: 'center', color: 'var(--text-muted)', fontSize: '0.82rem' }}>No predictions yet — click Refresh to generate AI tips.</div>
              ) : (() => {
                ...
                return Object.entries(groups).map(([dateStr, preds]) => {
                  ...
                  return (
                    <div key={dateStr}>
                      {/* Day divider */}
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', padding: '0.625rem 1.25rem', background: 'rgba(255,255,255,0.015)', borderBottom: '1px solid rgba(255,255,255,0.04)', borderTop: '1px solid rgba(255,255,255,0.04)' }}>
                        <span style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: '0.72rem', letterSpacing: '0.08em', color: isToday ? 'var(--accent)' : 'var(--text-secondary)', textTransform: 'uppercase' }}>{dayLabel}</span>
                        <span style={{ fontSize: '0.6rem', fontFamily: 'var(--font-mono)', color: 'var(--text-muted)', background: 'rgba(255,255,255,0.04)', padding: '0.1rem 0.4rem', borderRadius: '3px' }}>{d.toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })}</span>
                        <span style={{ fontSize: '0.6rem', color: isToday ? 'var(--accent)' : 'var(--text-muted)', background: isToday ? 'rgba(52,211,153,0.1)' : 'rgba(255,255,255,0.04)', border: isToday ? '1px solid rgba(52,211,153,0.2)' : 'none', borderRadius: '999px', padding: '0.1rem 0.5rem', fontFamily: 'var(--font-display)' }}>{preds.length} match{preds.length !== 1 ? 'es' : ''}</span>
                      </div>
                      {/* Rows for this day */}
                      {preds.map((p: any) => {
                          ...
                          <div key={p.id} style={{ display: 'grid', gridTemplateColumns: '1fr 200px 60px 60px 60px 100px', gap: '0.5rem', padding: '0.875rem 1.25rem', borderBottom: '1px solid rgba(255,255,255,0.03)', alignItems: 'center', cursor: 'pointer', transition: 'background 0.15s' }}
                            ...
                            <div>
                              ...
                              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                                ...
                                <span style={{ fontSize: '0.62rem', fontFamily: 'var(--font-mono)', color: 'var(--text-muted)', background: 'rgba(255,255,255,0.04)', padding: '0.1rem 0.375rem', borderRadius: '3px' }}>{matchTime}</span>
                              </div>
                            </div>
                            {/* AI probability bar */}
                            <div>
                              ...
                              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.55rem', fontFamily: 'var(--font-mono)' }}>
                            ...
                            {(['1', 'X', '2'] as const).map((type, oi) => {
                              const oddsVal = oi === 0 ? (p.homeOdds || 1.90) : oi === 1 ? (p.drawOdds || 3.20) : (p.awayOdds || 3.50);
                              const sel = isSelected(p.id, type);
                              const isAIPick = getOutcomeKey(p.prediction) === type;
                              return (
                                <button key={type} onClick={() => addBet(p, type, oddsVal)}
